    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-[#F5F0E8]/40 text-sm mt-1">{tours.length} tours in het aanbod</p>
        </div>
        <Link href="/admin/tours/new" className="px-5 py-2.5 bg-[#C9A84C] text-[#1C2B1E] text-xs tracking-[0.25em] uppercase hover:bg-[#E8C96A] transition-all duration-200">
          + Nieuwe tour
        </Link>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-[#1C2B1E] border border-[#C9A84C]/15 py-20 text-center">
          <p className="text-[#F5F0E8]/30 text-sm mb-4">Nog geen tours toegevoegd</p>
          <Link href="/admin/tours/new" className="text-[#C9A84C] text-xs tracking-wider hover:underline">
            Voeg je eerste tour toe →
          </Link>
        </div>
      ) : (
        <div className="space-y-3">
          {tours.map((tour) => (
            <div key={tour.id} className="bg-[#1C2B1E] border border-[#C9A84C]/15 hover:border-[#C9A84C]/30 transition-all duration-200 flex items-center gap-6 p-5">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-3 mb-1 flex-wrap">
                  ...
                  <span className={`text-[0.6rem] tracking-wider uppercase px-2 py-0.5 border ${
                    ...
                    {tour.published ? "Gepubliceerd" : "Verborgen"}
                  </span>
                </div>
                <div className="flex flex-wrap gap-4 text-[#F5F0E8]/40 text-xs">
                  <span>📍 {tour.location}</span>
                  <span>{tour.duration_hours}u</span>
                  <span>Max. {tour.max_guests} gasten</span>
                  <span className="text-[#C9A84C]">€{tour.price_per_person}/p.p.</span>
                </div>
              </div>
              <div className="flex items-center gap-2 shrink-0">
                ...
                >
                  {tour.published ? "Verberg" : "Publiceer"}
                </button>
                <Link
